...
try
{
    ...
    // ── Data Protection (Persist Keys in Database for Render Production) ──
    ...
    using (var scope = app.Services.CreateScope())
    {
        ...
        // Ensure DataProtectionKeys table exists dynamically for all db providers
        try
        {
            ...
            if (provider != null)
            {
                ...
                else if (provider.Contains("Npgsql") || provider.Contains("PostgreSQL"))
                {
                    await db.Database.ExecuteSqlRawAsync(
                        "CREATE TABLE IF NOT EXISTS \"DataProtectionKeys\" (" +
        ...
        {
            Log.Warning(ex, "Could not dynamically verify/create DataProtectionKeys table.");
        }
